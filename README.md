...
- 自动获取当前账号所有课程的作业数据
- 课程选择功能：首次加载弹出选择面板，支持搜索、全选/全不选/仅已开课，可随时通过工具栏「课程选择」按钮修改，选择状态跨会话持久化（建议不超过 12 门以避免风控）
- 按课程分组显示，支持展开/折叠（含批量展开/折叠按钮）
- 状态标签：未交（红）、待互评（紫）、待批阅（黄）、已完成（绿）、其他（灰）
  - 已互评归入已完成分类
- 筛选按钮：全部 / 未交 / 待互评 / 待批阅 / 已完成（状态跨会话持久化）
...
- 自动刷新：支持「页面加载时刷新」（默认开启）和「定时刷新」（可设 1-120 分钟间隔）
  - 页面加载刷新使用 sessionStorage + Navigation API 智能判断：F5 刷新时触发，从学习通打开新标签页时跳过
- 请求超时15秒，失败自动重试2次
